...
# date                    : 2020/02/04 14:18:33
# Turbine version         : 1.2.3
# Input filename          : /pfs/data3/project/fh1-project-devel/th7356/work/GITLAB/grk2450-p6-workflows/wf-systems/swift-t/matrix/tmm_tutorial/test-b.swift
# Output filename         : /pfs/data3/project/fh1-project-devel/th7356/work/GITLAB/grk2450-p6-workflows/wf-systems/swift-t/matrix/tmm_tutorial/test-b.tcl
...
# stc.ic.output-file            : 
# stc.input_filename            : test-b.swift
# stc.log.file                  : 
# stc.log.trace                 : false
...
# stc.output_filename           : test-b.tcl
...
package require b 0.0


proc swift:main {  } {
    turbine::c::log "function: __entry"
    # Var: file u:data test-b.swift:7:0
    # Var: blob u:v test-b.swift:8:0
    # Var: float[int] u:sum test-b.swift:10:0
    # Var: $file v:data VALUE_OF [file:data]
    lassign [ adlb::multicreate [ list blob 1 1 2 ] [ list container integer float 1 1 3 ] ] u:v u:sum
    turbine::c::log "allocated u:v=<${u:v}> u:sum=<${u:sum}>"
    lassign [ turbine::make_file_tds [ adlb::multicreate [ list file 1 1 1 ] ] [ list 0 ] ] u:data
    set tcltmp:init_rc 1
    set v:data [ turbine::create_local_file_ref "" ${tcltmp:init_rc} 0 ]
    turbine::input_file_local v:data "input.data"
    turbine::store_file ${u:data} v:data 1
    turbine::blob_read [ list ${u:v} ] [ list ${u:data} ]
    set a:t:8 [ adlb::subscript_container ${u:sum} 0 ]
    turbine::rule [ list ${u:v} ] "b-argwait ${u:sum} ${u:v}"
    turbine::rule [ list ${a:t:8} ] "__entry-call_foreign-printf {${a:t:8}}"
    turbine::decr_local_file_refcount v:data
}


proc b-argwait { u:sum u:v } {
    # Var: $blob v:v RENAMED [$blob:__v:v]
    # Var: $blob v:sum RENAMED [$blob:__v:sum]
    # Var: $float$[int] v:f RENAMED [$float$[int]:__v:f]
    set v:v [ turbine::retrieve_blob ${u:v} 1 ]
    set v:sum [ b::b_tcl ${v:v} ]
    set v:f [ turbine::blob2floats_impl ${v:sum} ]
    turbine::build_rec ${u:sum} ${v:f} [ list container integer float ] 0 1
    turbine::free_local_blob ${v:v}
    turbine::free_local_blob ${v:sum}
}


proc __entry-call_foreign-printf { a:t:8 } {
    # Var: $float v:alias:t:8 VALUE_OF [float:__alias:t:8]
    # Var: $void v:t:6 VALUE_OF [void:__t:6]
    set v:alias:t:8 [ turbine::retrieve_float ${a:t:8} CACHED 1 ]
    set v:t:6 [ turbine::printf_local "sum (swift): %f" ${v:alias:t:8} ]
}
...
adlb::add_debug_symbol 1 "data" "test-b:7:0"
adlb::add_debug_symbol 2 "v" "test-b:8:0"
adlb::add_debug_symbol 3 "sum" "test-b:10:0"
